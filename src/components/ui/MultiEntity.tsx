...
const MultiEntity: React.FC<{ entityData: any, schema:any}> = ({ entityData,schema }) => {
  ...
  return (
    <div>
      <table>
        <thead key="thead">
          <tr>{schema.map((c:any, i:any)=>{return <th style={{width:c.width }} key={`th_${c.name}_${i}`}>{c.displayName}</th>})}</tr>
        </thead> 
        <tbody key="tbody">
          {data.map((d:any,i:any)=>{return <tr key={`tr_${i}`}  className={d.id==editRowId?'addRow':''} >{schema.map((c:any, j:any)=>
            {return <td style={{width:c.width}} key={`td_${i}_${j}`} className={c.type=='icon'?'pl-0':''} >
              {((isAddMode(d.id) && c.name=='add') || (c.type!='icon' && d.id==editRowId) || (!isAddMode(d.id) && c.name=='delete'))? getDataItem(c.name,c.type,c.width,d.id): d[c.name]}
              </td>})}</tr>})}
        </tbody>
      </table>
    </div>

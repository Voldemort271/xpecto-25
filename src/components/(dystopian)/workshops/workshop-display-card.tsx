...
interface Props {
  comp: WorkshopWithDetails;
}


const sharetech = Share_Tech({ weight: "400", subsets: ["latin"] });

const WorkDisplayCard = ({ comp }: Props) => {
    ...
    >
      ...
      >
        <Image
          ...
          alt={comp.workshopDetails.name || "Untitled Event"}
          width={500}
          height={500}
      ...
      <div className="mt-12 flex w-full flex-col gap-5 md:mt-0">
        ...
        >
          {comp.workshopDetails.name || "Upcoming Event"}
        </motion.div>
        <motion.div
        ...
        >
          <div className="hidden md:block lg:hidden">
            {comp.workshopDetails.description.slice(0, 250)}
            ...
          </div>
          <div className="hidden lg:block xl:hidden">
            {comp.workshopDetails.description.slice(0, 400)}
            ...
          </div>
        ...
        >
          <MarqueeContainer
            text={[
              "view all details",
              comp.workshopDetails.name || "Upcoming Event",
              comp.workshopDetails.begin_time.toLocaleString() +
                " to " +
                comp.workshopDetails.end_time.toLocaleString(),
              "register for " +
                (comp.workshopDetails.name || "upcoming event"),
            ]}
            href={`/competitions/${comp.workshopDetails.slug}`}
          />
        </motion.div>
      </div>
    </motion.div>
